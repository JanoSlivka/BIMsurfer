...
export class SectionPlane {
    ...
    constructor(params) {
        ...
        this.sectionPlaneHelper = this.viewer.sectionPlaneHelper;
    ...
    position(coordinates, normal) {
        ...
        this.ps = [_tmp_sectionA, _tmp_sectionB, _tmp_sectionD, _tmp_sectionC, _tmp_sectionA];
        if (this.Poly) {
            this.Poly.points = this.ps;
        } else {
            this.Poly = this.viewer.overlay.createWorldSpacePolyline(this.ps, this.sectionPlaneHelper);
    ...
    enable(canvasPos, coordinates, normal, depth) {
        ...
        normal = this.sectionPlaneHelper.getNormalSectionPlane(normal);
        this.sectionPlaneHelper.saveLastNormalSectionPlane(normal);
        const depth = (this.sectionPlaneHelper.isFreeSectionIndex())
            ? vec3.dot(coordinates, normal)
            : this.ps[0][this.sectionPlaneHelper.sectionIndex];
        this.values.set(normal.subarray(0, 3));
        this.initialSectionPlaneD = this.values[3] = depth;
